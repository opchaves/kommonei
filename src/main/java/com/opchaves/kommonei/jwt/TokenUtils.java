...
public class TokenUtils {
  ...
  public static String generateToken(TokenPayload params) throws Exception {
    ...
    claims.issuer(params.issuer);
    claims.subject(params.id);
    claims.issuedAt(currentTimeInSecs);
    claims.expiresAt(currentTimeInSecs + params.duration);
    claims.groups(groups);
    claims.claim("email", params.email);

    return claims.jws().keyId(privateKeyLocation).sign(privateKey);
  }
